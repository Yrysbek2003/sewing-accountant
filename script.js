const DataManager = {
    users: JSON.parse(localStorage.getItem('users')) || {
        admin: {
            password: '12345',
            data: { purchases: [], sales: [], salaries: [], expenses: [], history: [], workers: [], tasks: [] }
        }
    },
    currentUser: null,
    ...
    SOCIAL_RATE: 0.2725,
    VAT_RATE: 0.12,
    clothingTypes: JSON.parse(localStorage.getItem('clothingTypes')) || {
    ...
    saveData() {
        ...
        localStorage.setItem('clothingTypes', JSON.stringify(this.clothingTypes));
    },

    logAction(action, details) {
        this.users[this.currentUser].data.history.push({ action, details, timestamp: new Date().toISOString() });
        this.saveData();
    },
    ...
    async fetchExchangeRates() {
        try {
            ...
            this.exchangeRates = {
                ...
                CNY: data.rates.CNY ? 1 / data.rates.CNY : this.exchangeRates.CNY,
    ...
    registerUser(username, password) {
        if (this.users[username]) {
            throw new Error(Translations[UIManager.currentLanguage].register_exists);
        }
        if (password.length < 5) {
            throw new Error(Translations[UIManager.currentLanguage].register_password_short);
        }
        this.users[username] = {
            password,
            data: { purchases: [], sales: [], salaries: [], expenses: [], history: [], workers: [], tasks: [] }
        ...
        this.saveData();
        this.logAction('register_user', { username });
    },
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (data.users && data.clothingTypes) {
...
const UIManager = {
    ...
    login() {
        ...
        if (DataManager.users[username] && DataManager.users[username].password === password) {
            DataManager.currentUser = username;
    ...
    addPurchase() {
        ...
        if (item && quantity && cost) {
            ...
            DataManager.logAction('add_purchase', purchase);
    ...
    deletePurchase(index) {
        ...
        li.addEventListener('animationend', () => {
            const [deleted] = data.purchases.splice(index, 1);
            DataManager.logAction('delete_purchase', deleted);
            this.updatePurchaseList(data.purchases);
            DataManager.saveData();
    ...
    addSale() {
        ...
        if (item && quantity && amount) {
            ...
            DataManager.logAction('add_sale', sale);
    ...
    deleteSale(index) {
        ...
        li.style.animation = 'fadeOut 0.3s ease';
        li.addEventListener('animationend', () => {
            const [deleted] = data.sales.splice(index, 1);
            DataManager.logAction('delete_sale', deleted);
            this.updateSaleList(data.sales);
            DataManager.saveData();
